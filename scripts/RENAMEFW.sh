   ...
   for pattern in "${search_replace_patterns[@]}"; do
        ...
        for file in *"${search}"*.img.gz; do
            if [[ -f "$file" ]]; then
                ...
                local new_name
                if [[ -n "$kernel" ]]; then
                    new_name="${OP_BASE}-${BRANCH}-${replace}-${kernel}-${TUNNEL}-${DATE}.img.gz"
                else
                    new_name="${OP_BASE}-${BRANCH}-${replace}-${TUNNEL}-${DATE}.img.gz"
                fi
        ...
        for file in *"${search}"*.tar.gz; do
            if [[ -f "$file" ]]; then
                local new_name
                new_name="-@sogek1ng__-${OP_BASE}-${BRANCH}-${replace}-${TUNNEL}-${DATE}.img.gz"
                echo -e "${INFO} Renaming: $file → $new_name"
                mv "$file" "$new_name" || {
                    echo -e "${WARN} Failed to rename $file"
                    continue
